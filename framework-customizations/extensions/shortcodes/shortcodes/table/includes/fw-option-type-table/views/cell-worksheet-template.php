...
<div class="getleads-table-cell-content <?php echo esc_attr($row_name) ?> <?php echo  $current_row_name === $row_name ? 'getleads-active-content' : '' ?>">
		...
		<?php foreach ( $option['content_options'][ $row_name ] as $key => $options ) : ?>
			...
			<?php if ( $row_name == 'button-row' ): ?>
				<?php $options['button'] = __( 'Edit', 'getleads' );
				if ( empty( $cell_value['button'] ) ) {
					$options['button'] = __( 'Add', 'getleads' );
				} ?>
			<?php endif; ?>

			<?php $div_attr = isset( $options['wrapper_attr'] ) ? $options['wrapper_attr'] : array(); ?>
			<?php $div_attr['class'] = isset( $div_attr['class'] ) ? $div_attr['class'] . ' getleads-cell-option-wrapper ' : ' getleads-cell-option-wrapper ' ?>

			<div <?php echo getleads_attr_to_html( $div_attr ) ?>>
				<?php echo getleads()->backend->option_type( $options['type'] )->render( $key . '-' . $i . '-' . $j, $options, $data_options ); ?>
			</div>
